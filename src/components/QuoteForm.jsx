  ...
  if (submitted) return (
    <div style={{background:'#fff',borderRadius:24,padding:40,textAlign:'center',boxShadow:'0 12px 48px rgba(0,0,0,0.12)',maxWidth:520,margin:'0 auto'}}>
      ...
      <p style={{color:'#555',marginBottom:8,lineHeight:1.7}}>
        We'll call you <strong>30 minutes before arrival</strong> on <strong>{form.day?.full}</strong> during the <strong>{form.timeSlot}</strong> window.
      </p>
      <div style={{background:greenLt,borderRadius:16,padding:20,margin:'20px 0',border:`1.5px solid rgba(45,122,58,.2)`}}>
        <div style={{fontFamily:'Barlow Condensed,sans-serif',fontSize:'1.1rem',fontWeight:700,color:green,marginBottom:4}}>Your Appointment</div>
      ...
      <a href="tel:9043341521" style={{display:'block',fontFamily:'Barlow Condensed,sans-serif',fontSize:'1.5rem',fontWeight:700,color:green,margin:'16px 0'}}>
        📞 (904) 334-1521
      </a>
      <button onClick={()=>{setSubmitted(false);setStep(1);setForm({type:'Home',zip:'',service:'',day:null,timeSlot:null,firstName:'',lastName:'',phone:'',email:'',address:'',message:''});}}
        style={{background:'none',border:`2px solid ${green}`,color:green,borderRadius:50,padding:'10px 28px',fontFamily:'Barlow Condensed,sans-serif',fontSize:'1.1rem',fontWeight:700,cursor:'pointer'}}>
        Book Another
      </button>
    </div>
  );
      ...
      {step === 1 && (
        <div>
          ...
          <div style={{display:'flex',gap:12,marginBottom:24}}>
            {['Home','Business'].map(t=>(
              <button key={t} onClick={()=>set('type',t)} style={{
                ...
                color: form.type===t ? (t==='Home'?green:purple) : '#999',
                transition:'all .2s',
              }}>
                {t === 'Home' ? '🏠' : '🏢'} {t}
              </button>
          ...
          <div style={{marginBottom:20}}>
            ...
            />
            {form.zip.length >= 5 && (
              <div style={{color:green,fontSize:'.85rem',fontWeight:600,marginTop:6}}>✅ We serve your area!</div>
            )}
          </div>
            ...
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}>
              {SERVICES.map(s=>(
                <button key={s} onClick={()=>set('service',s)} style={{
                  ...
                  color:form.service===s?green:'#555',
                  transition:'all .2s',
                }}>{s}</button>
              ))}
            </div>
          ...
          <button onClick={()=>setStep(2)} disabled={!canNext1} style={{
            width:'100%',padding:16,background:canNext1?green:'#e8e8e8',
            color:canNext1?'#fff':'#999',border:'none',borderRadius:50,
            fontFamily:'Barlow Condensed,sans-serif',fontSize:'1.2rem',fontWeight:700,
            cursor:canNext1?'pointer':'not-allowed',transition:'all .2s',
          }}>
...
<div style={{marginBottom:24}}>
  ...
  <CalendarPicker selected={form.day} onSelect={d=>set('day',d)} today={today} green={green} greenLt={greenLt}/>
          ...
          <div style={{display:'flex',gap:12}}>
            <button onClick={()=>setStep(1)} style={{flex:1,padding:16,background:'#fff',color:'#555',border:'2px solid #e8e8e8',borderRadius:50,fontFamily:'Barlow Condensed,sans-serif',fontSize:'1.1rem',fontWeight:700,cursor:'pointer'}}>
              ← Back
            </button>
            <button onClick={()=>setStep(3)} disabled={!canNext2} style={{
              flex:2,padding:16,background:canNext2?green:'#e8e8e8',
              color:canNext2?'#fff':'#999',border:'none',borderRadius:50,
              fontFamily:'Barlow Condensed,sans-serif',fontSize:'1.2rem',fontWeight:700,
              cursor:canNext2?'pointer':'not-allowed',transition:'all .2s',
            }}>
              Next: Your Info →
            </button>
          </div>
      ...
      {step === 3 && (
        <div>
          ...
          <div style={{display:'flex',gap:12}}>
            <button onClick={()=>setStep(2)} style={{flex:1,padding:16,background:'#fff',color:'#555',border:'2px solid #e8e8e8',borderRadius:50,fontFamily:'Barlow Condensed,sans-serif',fontSize:'1.1rem',fontWeight:700,cursor:'pointer'}}>
              ← Back
            </button>
            <button onClick={handleSubmit} disabled={!canSubmit||submitting} style={{
              flex:2,padding:16,
              background:canSubmit&&!submitting?green:'#e8e8e8',
              color:canSubmit&&!submitting?'#fff':'#999',
              border:'none',borderRadius:50,
              fontFamily:'Barlow Condensed,sans-serif',fontSize:'1.2rem',fontWeight:700,
              cursor:canSubmit&&!submitting?'pointer':'not-allowed',transition:'all .2s',
            }}>
